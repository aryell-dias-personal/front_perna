...
class _InitialPageState extends State<InitialPage> {
  ...
  Widget build(BuildContext context) {
    return Center(
      child: isLoading
          ? SpinKitDoubleBounce(
              color: Theme.of(context).primaryColor, size: 100.0)
                    ...
                    if (user == null) {
                      showSnackBar(
                          AppLocalizations.of(context)
                              .translate('unrecognized_user'),
                          Colors.redAccent,
                          context);
                    } else {
                      store.dispatch(choice == SignLogin.sign
                          ? SignIn(user)
                          : LogIn(user));
                    }
                  },
              builder: (BuildContext context,
                      dynamic Function(SignLogin) onSignLogIn) =>
                  Column(
