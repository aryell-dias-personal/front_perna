...
class PaymentsService{
  PaymentsService({
    ...
    StripePayment.setOptions(
      StripeOptions(
        publishableKey: FlavorConfig.instance.variables['paymentPublishableKey'] as String,
        merchantId: FlavorConfig.instance.variables['merchantId'] as String, 
        androidPayMode: FlavorConfig.instance.variables['androidPayMode'] as String
      )
    );
  }
  ...
  Future<List<model.CreditCard>> listCard(String token) async { 
    ...
    if(res.statusCode == 200) {
      ...
      return response['retrivedCards'].map<model.CreditCard>(
        (dynamic parsedJson) => model.CreditCard.fromJson(parsedJson as Map<String, dynamic>)
  ...
  Future<int> deleteCard(String creditCardId, String token) async { 
    final dynamic body = await myDecoder.encode(<String, String>{
  ...
  Future<int> addCard(model.CreditCard creditCard, String token) async { 
    try {
      ...
      final Token tokenWithCard = await StripePayment.createTokenWithCard(CreditCard(
        number: cardNumber,
      ...
      final Response res = await post(
        ...
        body: await myDecoder.encode(<String, String>{
